...
# for f in lastFiles:
    # os.remove(f)
#subprocess.Popen(["D:\\users\\andylin77\\Documents\\NCHU\\ProjectFile\\PE_Golf\\Golf_Run\\run\\OpenPoseDemo.exe","--model_folder","D:\\users\\andylin77\\Documents\\NCHU\\ProjectFile\\PE_Golf\\Golf_Run\\models","--camera","0","--num_gpu_start","0","--process_real_time","-number_people_max","1","--net_resolution","480x368","--write_json","StudentData\\json\\"])
#time.sleep(1)

std = 1
swing = 3

#resultPath = "C:\\Users\\kelly\\Desktop\\NCHU-NLPLab-PEGolf\\OpenPose\\Golf_Run\\realTimeJudge2\\StudentScore\\Std2_{std}\\S2_{std}_{swing}.txt".format(std=std, swing=swing)
resultPath = "D:\\DataSet\\202306\\0601_1\\Student_1_21\\S2_{std}_{swing}.txt".format(std=std, swing=swing)
...
while True:
    ...
    fileName = "C:\\Users\\kelly\\Desktop\\NCHU-NLPLab-PEGolf\\OpenPose\\DataSet\\202306\\0601_2\\Student_2_{std}\\output\\json\\swing{swing}\\Student_2_{std}_{swing}_".format(std=std, swing=swing)
    ...
    while not os.path.exists(nextFileName): #下一個檔案存在才會去進行
        resultMsg += "Execute File Count: "
        resultMsg += str(fileNumber)
        resultMsg += "\n"
        resultMsg += "...Done"
        ...
        if len(data["people"]) > 0 :
            ...
            if swingState == 1: #已經檢測到準備完成，但可能還在準備動作
                if readyScore > READY_SCORE_THRESHOLD: #如果不是預備動作 (代表開始揮桿了)
                    ...
                    resultMsg += "Ready: "
                    resultMsg += readyPoseRecord
                    resultMsg += "\n"
                    resultMsg += "Swing Start: "
                    resultMsg += fileName
                    resultMsg += "\n"
